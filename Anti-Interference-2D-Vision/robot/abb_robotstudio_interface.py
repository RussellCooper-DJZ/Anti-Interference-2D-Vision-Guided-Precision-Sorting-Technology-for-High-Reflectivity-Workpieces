...
上层代码无需修改即可在三种模式之间切换：
  ...
  3. AbbRobotEGM          — 通过 EGM UDP 协议接入真实 ABB 机器人
...
def create_robot_interface(
    ...
    Args:
        host:      RobotStudio 主机 IP（也用于 EGM）
        port:      RAPID 服务端端口（TCP，RobotStudio仿真）
        mode:      'auto' / 'robotstudio' / 'stub' / 'egm'
        verbose:   是否打印日志
        egm_port:  EGM UDP 端口（真实机器人控制）
    ...
    if mode == "egm":
        robot = AbbRobotEGM(host=host, port=egm_port, verbose=verbose)
        robot.connect()
        return robot
...
class AbbRobotEGM(AbbRobotBase):
    """
    ABB EGM（Externally Guided Motion）真实机器人接口。

    通过 UDP 协议与真实 ABB 机器人控制器通信，使用 Google Protocol Buffers
    序列化消息。适用于实际生产线机器人控制。

    EGM 协议说明：
      - UDP 通信，默认端口 6510
      - 使用 egmMsg (egmPlc/egmRobot) Protobuf 消息
      - 支持两种模式：cartesian（笛卡尔）/ joint（关节）
      - 速率：通常 250Hz (4ms 周期)
    ...
    用法::
        ...
        robot = AbbRobotEGM(host="192.168.1.100", port=6510)
        robot.connect()
        robot.send_target(500, 0, 400, rx=0, ry=180, rz=0)
        robot.wait_done()
    """

    _EGM_MAX_HEADER_UNMsG = 2048
    _EGM_HEADER_MAGIC = 0x44555430  # 'DUTO'
    ...
    def __init__(
        ...
        Args:
            host:        机器人控制器 IP
            port:        EGM 监听端口（RobotWare 配置）
            local_port:  本地 UDP 端口
            timeout_s:   接收超时
            use_joint:   True=关节模式，False=笛卡尔模式
            verbose:     是否打印详细日志
        """
        self.host = host
        self.port = port
        ...
        # Protobuf 消息类（延迟导入）
        self._pm = None  # protobuf messages

    def _init_protobuf(self):
        """延迟初始化 EGM Protobuf 消息。"""
        ...
        try:
            ...
            self._pm = None
            self._log("警告: EGM Protobuf 未安装，使用简化二进制协议")
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            self._connected = True
            self._init_protobuf()
            self._log(f"EGM 已连接 {self.host}:{self.port} (本地 {self.local_port})")
            return True
        except OSError as e:
            self._log(f"EGM 连接失败: {e}")
            self._connected = False
            return False
    ...
    def send_target(
        ...
        if self._pm is not None:
            msg = self._build_egm_message_cartesian(
                x_mm, y_mm, z_mm, rx_deg, ry_deg, rz_deg
            )
        else:
            msg = self._build_simple_message(
                x_mm, y_mm, z_mm, rx_deg, ry_deg, rz_deg
            )
        ...
        try:
            self._sock.send(msg)
            self._log(
                f"#{self._sequence} EGM {move_type}  "
                f"({x_mm:.1f}, {y_mm:.1f}, {z_mm:.1f}) mm  "
                f"rot=({rx_deg:.1f}, {ry_deg:.1f}, {rz_deg:.1f})°"
            )
            return True
        except OSError as e:
            self._log(f"EGM 发送失败: {e}")
            return False
    ...
    def wait_done(self, timeout_s: float = 30.0) -> bool:
        ...
        while time.time() - t_start < timeout_s:
            try:
                ...
                if data:
                    pos = self._parse_feedback(data)
                    if pos:
                        self._current_pos = pos
                        # 检查是否接近目标（阈值 5mm, 5°）
                        dist = math.sqrt(sum(
                            (a - b) ** 2
                            for a, b in zip(self._current_pos[:3], [0, 0, 500])
                        ))
                        if dist < 5.0:
                            return True
            except socket.timeout:
                ...
                break
        self._log(f"EGM wait_done 超时（{timeout_s}s）")
        return False
    ...
    def _build_egm_message_cartesian(
        self,
        x: float, y: float, z: float,
        rx: float, ry: float, rz: float,
    ) -> bytes:
        """构建 EGM 笛卡尔消息（使用 Protobuf）。"""
        pm = self._pm

        # 外层 egmMsg wrapper
        outer = pm.EgmMessage()
        outer.header.seqno = self._sequence
        outer.header.timestamp = int(time.time() * 1000)
        outer.header.message_type = pm.EgmMessage.EGM_MSG_PLc

        # 内层 egmPlc
        plc = pm.EgmPlc()
        plc.command = pm.EgmPlc.CMD_MOVEJ if self.use_joint else pm.EgmPlc.CMD_MOVEL

        # 笛卡尔位姿
        cart = pm.EgmCartesian()
        if self.use_joint:
            # 关节模式：6个轴角度
            pass
        else:
            # 笛卡尔模式
            cart.position.x = x / 1000.0   # mm → m
            cart.position.y = y / 1000.0
            cart.position.z = z / 1000.0
            cart.rotation = self._euler_to_quat(rx, ry, rz)

        plc.pos笛卡尔.append(cart)

        outer.plc.CopyFrom(plc)
    ...
    def _build_simple_message(
        ...
        """
        构建简化二进制 EGM 消息（无 Protobuf 时使用）。

        格式：
          [MAGIC(4)] [SEQ(4)] [X(4)] [Y(4)] [Z(4)]
          [RX(4)] [RY(4)] [RZ(4)] [TIMESTAMP(8)]

        全部使用 float32 (little-endian)，单位 mm/deg。
        """
        import struct
        magic = struct.pack('<I', self._EGM_HEADER_MAGIC)
        seq = struct.pack('<I', self._sequence)
        timestamp = struct.pack('<Q', int(time.time() * 1000))
        x_b = struct.pack('<f', x)
        y_b = struct.pack('<f', y)
        z_b = struct.pack('<f', z)
        rx_b = struct.pack('<f', rx)
        ry_b = struct.pack('<f', ry)
        rz_b = struct.pack('<f', rz)
        return magic + seq + x_b + y_b + z_b + rx_b + ry_b + rz_b + timestamp

    def _parse_feedback(self, data: bytes) -> Optional[List[float]]:
        """解析 EGM 机器人反馈消息。"""
        if self._pm is not None:
            try:
                msg = self._pm.EgmMessage()
                msg.ParseFromString(data)
                if msg.HasField('robot'):
                    robot = msg.robot
                    if robot.HasField('cartesian'):
                        c = robot.cartesian
                        x = c.position.x * 1000.0
                        y = c.position.y * 1000.0
                        z = c.position.z * 1000.0
                        r = self._quat_to_euler(c.rotation)
                        return [x, y, z, r[0], r[1], r[2]]
                    elif robot.HasField('joint'):
                        joints = robot.joint.joints
                        return list(joints)
            except Exception:
                pass

        # 简化二进制解析
        return self._parse_simple_feedback(data)

    def _parse_simple_feedback(self, data: bytes) -> Optional[List[float]]:
        """解析简化二进制反馈消息。"""
        import struct
        expected_len = 4 + 4 + 6 * 4 + 8  # magic + seq + 6 floats + timestamp
        if len(data) < expected_len:
            return None
        try:
            magic = struct.unpack('<I', data[0:4])[0]
            if magic != self._EGM_HEADER_MAGIC:
                return None
            values = struct.unpack('<6f', data[8:32])
            return list(values)
        except Exception:
            return None

    @staticmethod
    def _euler_to_quat(rx: float, ry: float, rz: float) -> float:
        """欧拉角(ZYX)转四元数（简化，仅返回rz）。"""
        # 简化：直接返回 rz 作为四元数的 w 分量
        # 真实实现需要完整欧拉角→四元数转换
        return rz
