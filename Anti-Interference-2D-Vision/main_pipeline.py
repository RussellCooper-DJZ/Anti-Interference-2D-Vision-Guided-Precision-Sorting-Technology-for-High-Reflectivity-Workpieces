...
完整流水线：
  摄像头采集 → HDR 融合 → 反光抑制 → FLARE 推理
  → 像素级定位 → 坐标变换 → ABB 机器人通信（模拟桩）
...
from core import initialize_core, get_core_status

# 全局初始化状态
_core_initialized = False

def _init_core_if_needed():
    """延迟初始化核心组件（仅在需要时）"""
    global _core_initialized
    if _core_initialized:
        return
    initialize_core(
        project_name="AGEANet",
        log_level="INFO",
...
class VisionInferenceEngine:
    """
    封装 FLARE 推理，支持 CPU/GPU，输入 BGR 图像，输出分割掩膜和边缘图。
    """
    ...
    def __init__(self, model_path: Optional[str] = None,
        ...
        # 构建模型
        if model_type == 'lite':
            self.model = FLARELite(in_channels=3, base_ch=base_ch).to(self.device)
        else:
            self.model = FLARE(in_channels=3, base_ch=base_ch).to(self.device)
...
class ShipVisionPipeline:
    ...
    def __init__(
        ...
        use_scratch_detect: bool = False,
        use_measurement: bool = True,
    ):
        self.verbose = verbose
        self.use_photometric = use_photometric
        self.use_scratch_detect = use_scratch_detect
        ...
        # 光度立体（用于高光抑制增强）
        if use_photometric:
            self.photometric = PhotometricStereo()

        # 划痕检测器
        if use_scratch_detect:
            self.scratch_detector = ScratchDetector()

        # 边缘缺陷检测器
        self.edge_defect_detector = EdgeDefectDetector()
        ...
        if use_measurement:
            self.caliper = CaliperMeasurement(
                ...
                polarity='black_to_white',
                edge_intensity=20,
                search_line_count=20,
            )
            self.gap_measurer = GapMeasurement(
                search_direction='left_to_right',
                polarity='black_to_white',
                edge_intensity=20,
                edge_spacing=5,
    ...
    def process_frame(
        ...
        multi_light_images: Optional[List[np.ndarray]] = None,
    ) -> Dict:
        """
        处理单帧图像，返回完整分析结果。
        ...
        Args:
            ...
            multi_light_images: 多光源图像列表（可选，用于光度立体）
        ...
        Returns:
            Dict 包含：
              ...
              edge_mask:     边缘掩膜
              glare_mask:    高光区域掩膜
              detections:    目标列表（含机器人坐标、测量结果）
              vis_image:     可视化结果图
              timing:        各阶段耗时（ms）
              photometric:    光度立体结果（如果启用）
              scratch_result: 划痕检测结果（如果启用）

        detections 每项新增字段：
              caliper_distance_px: 卡尺测量距离（像素）
              caliper_distance_mm: 卡尺测量距离（mm，有深度图时）
              caliper_valid:       卡尺测量是否有效
              gap_pitches:         节距列表（像素）
              gap_pitches_mm:      节距列表（mm，有深度图时）
              gap_widths:         宽度列表（像素）
              gap_spacings:       间距列表（像素）
        """
        t_start = time.perf_counter()
        timing  = {}

        # ---- Step 1: HDR 融合 + 反光抑制（使用配置的AntiGlarePipeline）----
        t0 = time.perf_counter()
        if image_over is not None:
            # 双曝光HDR + 反光抑制
            image_hdr = self.anti_glare.process_multi([image_bgr, image_over])
        else:
            # 单张图像：自动生成多曝光序列处理
        ...
        # ---- Step 2: 光度立体（可选）----
        photometric_result = None
        if self.use_photometric and multi_light_images is not None:
            t0 = time.perf_counter()
            try:
                photometric_result = self.photometric.process(multi_light_images)
                # 使用光度立体法线指导高光修复
                if photometric_result is not None:
                    image_hdr = self._apply_photometric_guidance(
                        image_hdr, photometric_result
                    )
                timing['photometric_ms'] = (time.perf_counter() - t0) * 1000
            except Exception as e:
                if self.verbose:
                    print(f"[Pipeline] 光度立体处理失败: {e}")
                timing['photometric_ms'] = (time.perf_counter() - t0) * 1000
        else:
            timing['photometric_ms'] = 0.0

        # ---- Step 3: 反光抑制（已集成到Step1的AntiGlarePipeline中）----
        t0 = time.perf_counter()
        # AntiGlarePipeline.process_multi 已经包含了高光修复和镜面反射抑制
        # 此处仅做高光区域检测（用于后续排除）
        ...
        # ---- Step 4: 深度学习推理 ----
        t0 = time.perf_counter()
        # image_hdr 已经是 AntiGlarePipeline 处理后的图像
        seg_mask, edge_mask = self.engine.infer(image_hdr)
        ...
        # ---- Step 6: 像素级定位 ----
        t0 = time.perf_counter()
        # 确保灰度图（处理HDR可能产生单通道的情况）
        if image_hdr.ndim == 3:
            gray = cv2.cvtColor(image_hdr, cv2.COLOR_BGR2GRAY)
        else:
            gray = image_hdr if image_hdr.ndim == 2 else image_hdr[:, :, 0]
        ...
        if self.use_measurement:
            ...
            for det in raw_results:
                ...
                # 卡尺测量（物体宽度）
                caliper_result = self.caliper.measure(gray, roi)
                det['caliper_distance_px'] = caliper_result['distance']
                det['caliper_valid'] = caliper_result['valid']

                # 间隙测量（多边缘节距）
                gap_result = self.gap_measurer.measure(gray, roi)
                det['gap_pitches'] = gap_result['pitches']
                det['gap_widths'] = gap_result['widths']
                det['gap_spacings'] = gap_result['spacings']

                # 如果有深度图，转换为物理尺寸（mm）
                if depth_map is not None:
                    px_to_mm_x = (plane_d_mm / self.transformer.K[0, 0])
                    px_to_mm_y = (plane_d_mm / self.transformer.K[1, 1])
                    if caliper_result['valid'] and caliper_result['distance'] > 0:
                        det['caliper_distance_mm'] = (
                            caliper_result['distance'] * (px_to_mm_x + px_to_mm_y) / 2
                        )
                    if gap_result['pitches']:
                        det['gap_pitches_mm'] = [
                            p * (px_to_mm_x + px_to_mm_y) / 2 for p in gap_result['pitches']
                        ]
        timing['measure_ms'] = (time.perf_counter() - t0) * 1000
        ...
        # ---- Step 8: 划痕检测（可选）----
        scratch_result = None
        if self.use_scratch_detect:
            t0 = time.perf_counter()
            try:
                scratch_result = self.scratch_detector.detect(image_hdr)
                timing['scratch_ms'] = (time.perf_counter() - t0) * 1000
            except Exception as e:
                if self.verbose:
                    print(f"[Pipeline] 划痕检测失败: {e}")
                scratch_result = None
                timing['scratch_ms'] = 0.0
        else:
            timing['scratch_ms'] = 0.0

        # ---- Step 9: 机器人通信 ----
        t0 = time.perf_counter()
        if send_to_robot and self.robot.is_connected():
        ...
        vis = self._visualize(image_hdr, seg_mask, edge_mask,
                               glare_mask, detections, scratch_result)
        ...
        if self.verbose:
            print(f"[Pipeline] 帧 #{self._frame_count:04d}  "
                  ...
                  f"测量={timing.get('measure_ms', 0):.0f})")
    ...
    def _apply_photometric_guidance(
        self,
        image: np.ndarray,
        photometric_result: Dict,
    ) -> np.ndarray:
        """
        使用光度立体法线结果指导图像增强。
        ...
        Args:
            ...
            photometric_result: 光度立体输出，包含法线和反照率

        Returns:
            增强后的图像
        """
        normal = photometric_result.get('normal')
        if normal is None:
            return image
        ...
        # 对法线变化剧烈的区域进行细节增强
        edge_strength_norm = (edge_strength / (edge_strength.max() + 1e-8) * 255).astype(np.uint8)

        # 使用边缘信息指导锐化
        gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
        enhanced = cv2.addWeighted(gray, 0.8, edge_strength_norm * 0.5, 0.2, 0)
    ...
    def _visualize(
        ...
        scratch_result: Optional[Dict] = None,
    ) -> np.ndarray:
        ...
        if scratch_result is not None:
            scratch_mask = scratch_result.get('scratch_mask')
            if scratch_mask is not None:
                vis_result[scratch_mask > 0] = [255, 200, 200]
...
def run_demo_mode(pipeline: ShipVisionPipeline, output_dir: str):
    """演示模式：使用合成图像运行流水线。"""
    try:
        from data.synth_dataset_generator import synthesize_one_sample, set_seed
    except ImportError:
        # 尝试从当前目录导入
        import sys
