...
class PBRLightingSystem:
    """
    基于物理渲染（PBR）的高光金属光照系统。

    使用 Blinn-Phong BRDF 模型计算镜面反射，相比原有加法高光椭圆
    更真实地模拟：高光强度随视角/光源方向变化的物理特性。

    BRDF 模型：
      f(l, v) = kd·(albedo/π) + ks·(D·F·G) / (4·(n·v)·(n·l))
    其中：
      D = Blinn-Phong 微平面分布
    ...
    def __init__(
        self,
        roughness: float = 0.1,      # 0.01(镜面) ~ 1.0(漫反射)
        metallic: float = 0.95,       # 0.0(非金属) ~ 1.0(纯金属)
        light_intensity: float = 2.0, # 光源强度
        light_angle_deg: float = 45.0,# 光源方位角
        light_elevation_deg: float = 30.0,  # 光源仰角
        specular_scale: float = 3.0, # 高光强度缩放
    ):
        self.roughness = max(0.01, roughness)
        ...
        # 预计算光源方向向量
        la = math.radians(light_angle_deg)
        le = math.radians(light_elevation_deg)
        self._L = np.array([
        ...
        # Schlick Fresnel 参数（金属根据经验调整）
        self._F0 = metallic * 0.9 + 0.04

    def compute_normal_map(self, gray_image: np.ndarray) -> np.ndarray:
        """
        从灰度图像估计表面法线（使用Sobel梯度）。

        Args:
            gray_image: (H, W) 灰度图

        Returns:
            normal_map: (H, W, 3) 单位法线向量
        """
        # Sobel 梯度
        ...
        nz = np.ones_like(gray_image, dtype=np.float32) * 2.0  # 缩放z以控制敏感度
        nx = -gx / (nz + 1e-6)
        ny = -gy / (nz + 1e-6)
    ...
    def compute_specular(
        ...
        Args:
            normal_map: (H, W, 3) 单位法线向量
            view_direction: (3,) 视点方向，默认正对相机 (0,0,1)

        Returns:
            specular: (H, W) 镜面反射强度 [0, 1]
        """
        ...
        if view_direction is None:
            ...
            V = np.array(view_direction, dtype=np.float64)
            V /= (np.linalg.norm(V) + 1e-8)

        # 光源方向
        L = self._L.astype(np.float64)
        N = normal_map.astype(np.float64)
        ...
        # 点积
        NdotL = np.clip(N.dot(L), 0, 1).astype(np.float32)
        NdotH = np.clip(np.sum(N * H, axis=-1), 0, 1).astype(np.float32)
        NdotV = np.clip(N.dot(V), 0, 1).astype(np.float32)

        # Blinn-Phong 分布 D
        shininess = 2.0 / (self.roughness ** 2) - 2.0
        D = ((shininess + 2.0) / (2.0 * math.pi)) * np.power(NdotH, shininess)
        D = np.clip(D, 0, 10).astype(np.float32)

        # Schlick Fresnel F
        F = self._F0 + (1.0 - self._F0) * np.power(1.0 - NdotH, 5)

        # 粗糙度几何遮蔽 G（简化版）
        r = self.roughness
        G = np.clip(NdotL * NdotV / (NdotV + NdotL + 1e-8), 0, 1).astype(np.float32)

        # 最终镜面强度
        specular = D * F * G / (4.0 * NdotV + 1e-8)
        specular = np.clip(specular * self.specular_scale * self.light_intensity, 0, 1)
    ...
    def apply(self, image: np.ndarray) -> np.ndarray:
        ...
        Args:
            image: (H, W, 3) BGR uint8 输入

        Returns:
            BGR uint8 输出（含真实镜面高光）
        """
        gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY).astype(np.float32)
        normal_map = self.compute_normal_map(gray)
        specular = self.compute_specular(normal_map)

        # 高光叠加到原图
        result = image.astype(np.float32)
        specular_3ch = specular[:, :, np.newaxis]
        # 高光偏白（金属表面反射光源本身颜色）
        result += specular_3ch * 200.0 * self.light_intensity
...
LIGHTING_MODES = [
    'direct_sun', 'overcast', 'water_reflect', 'floodlight', 'mixed',
    # PBR 模式（基于 Blinn-Phong BRDF 的物理高光）
    'pbr',           # 纯 PBR 高光
    'pbr_sun',       # PBR + 方向光
    'pbr_mixed',     # PBR + 方向光 + 水面反射
]
...
def synthesize_one_sample(
    ...
    # --- 光照 ---
    if lighting_mode.startswith('pbr'):
        # PBR 模式：使用基于 Blinn-Phong BRDF 的物理高光
        light_angle = float(RNG.uniform(20, 160))
        light_elevation = float(RNG.uniform(15, 75))
        specular_scale = float(RNG.uniform(2.0, 5.0))
        pbr_roughness = float(RNG.uniform(0.02, 0.3))
        pbr = PBRLightingSystem(
            roughness=pbr_roughness,
            metallic=0.9,
            light_intensity=2.0,
            light_angle_deg=light_angle,
            light_elevation_deg=light_elevation,
            specular_scale=specular_scale,
        )
        canvas = pbr.apply(canvas)
        if lighting_mode in ('pbr_mixed',):
            # 叠加水面反射
            water = LightingSystem(mode='water_reflect')
            canvas = water.apply(canvas)
    else:
