...
## 6. Enhanced PBR Lighting System (2026-04-23 Update)

The hardware design now integrates with the PBRLightingSystem for enhanced specular reflection simulation:

### 6.1 Hardware Requirements for PBR Mode

| Component | Specification | Purpose |
|-----------|---------------|---------|
| **Light Source** | Multi-directional LED array with adjustable intensity | Supports `pbr_sun` mode with directional sunlight simulation |
| **Camera** | Global shutter CMOS, ≥3 exposure brackets | Multi-exposure HDR capture for PBR training data |
| **Computing** | NVIDIA GPU for training, ONNX Runtime for edge | TensorRT FP16 ~10ms inference on edge |

### 6.2 PBR Physical Parameters

| Parameter | Hardware Implication | Value Range |
|-----------|---------------------|-------------|
| `roughness` | Surface finish selection | 0.01 (mirror-like) ~ 1.0 (diffuse) |
| `metallic` | Material type | 0.0 (non-metal) ~ 1.0 (pure metal) |
| `specular_scale` | Light source brightness | 0.0 ~ 2.0 |

### 6.3 Software-Defined Lighting Modes

The PBR system supports multiple lighting modes configurable via software:

| Mode | Description | Hardware Requirement |
|------|-------------|---------------------|
| `pbr` | Standard Blinn-Phong BRDF | Basic LED array |
| `pbr_sun` | PBR + directional sunlight | High-intensity directional LED |
| `pbr_mixed` | PBR + water reflection | Diffuse + reflective surface setup |
