...
## 5. Algorithm Enhancement for Embedded Deployment (2026-04-23 Update)

### 5.1 PBR Glare Physical Simulation for Embedded

For embedded deployment on RA8P1, the PBR lighting system uses Blinn-Phong BRDF model with physical parameters optimized for edge inference:

| Parameter | Embedded Optimization | Value Range |
|-----------|----------------------|-------------|
| `roughness` | Quantized to 8-bit | 0.01 (mirror) ~ 1.0 (diffuse) |
| `metallic` | Quantized to 8-bit | 0.0 (non-metal) ~ 1.0 (pure metal) |
| `D/F/G Terms` | Fixed-point arithmetic | Smith geometric masking |

Supported modes: `pbr` / `pbr_sun` / `pbr_mixed`

### 5.2 PhotometricStereoNet for Embedded

The PhotometricStereoNet CNN directly regresses normals/albedo, avoiding the MIT US6,477,268 least-squares patent:

- **Input**: Multi-exposure HDR images (3 frames)
- **Output**: Surface normals + Albedo maps
- **Architecture**: Lightweight U-Net style CNN (~500K parameters)
- **Quantization**: INT8 inference support for RA8P1 Helium acceleration

### 5.3 Three-Backend Inference Options

For embedded scenarios, the system supports multiple inference backends:

| Engine | RA8P1 Suitability | Latency | Notes |
|--------|-------------------|---------|-------|
| PyTorch FP32 | Limited (no GPU) | ~100ms | Research only |
| ONNX Runtime | **Recommended** | ~40ms | Cross-platform, Helium-optimized |
| TensorRT FP16 | N/A | N/A | Requires NVIDIA GPU, not for embedded |

**Recommended**: ONNX Runtime with INT8 quantization for RA8P1 deployment.

### 5.4 Patent-Compliant Edge Detection

The embedded system implements patent-compliant alternatives:

| Feature | Embedded Implementation | Avoided Patent |
|---------|------------------------|----------------|
| Gray Matching | SSDA (TM_SQDIFF_NORMED) | Cognex US6,041,139 |
| Hand-Eye Calibration | PnP+RANSAC | AX=XB equation patent |

---

## 6. Conclusion and Outlook

Porting the high-reflectivity workpiece vision-guided sorting technology to the Renesas RA8P1 platform, along with the 2026-04-23 embedded-specific optimizations (PBR simulation, PhotometricStereoNet, ONNX Runtime deployment, and patent-compliant implementations), will significantly reduce hardware costs and improve system integration. By deeply optimizing algorithms and fully utilizing hardware acceleration (Helium MVE), it is expected to achieve industrial-grade performance in embedded environments. Future work will include specific model conversion, Helium optimized code implementation, FSP driver development, and system-level performance testing.
