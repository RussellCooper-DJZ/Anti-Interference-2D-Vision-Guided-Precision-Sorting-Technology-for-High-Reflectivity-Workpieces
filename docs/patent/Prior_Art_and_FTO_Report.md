...
- **本项目的创新**: 现有技术多依赖辅助标记（Fiducial Markers）或传统几何特征。本项目提出的 **AGEANet** 通过深度学习直接从高反光图像中提取边缘，且引入了**边缘感知分支**，在无标记情况下实现了更高的鲁棒性。
...
### 2.1 规避核心算法专利
- **规避策略**: 针对现有 U-Net 或注意力机制的通用专利，本项目的 **AGEANet** 采用了独特的双分支架构（分割+边缘感知），其损失函数设计和特征融合方式具有独立知识产权，不落入现有通用图像分割专利的保护范围。
