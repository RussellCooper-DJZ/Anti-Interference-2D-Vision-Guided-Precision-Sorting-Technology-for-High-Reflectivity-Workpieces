...
用法::
    ...
    from vision.existence_checking import BlobAnalyzer, GrayMatcher, FeatureMatcher, ContourMatcher

    # Blob分析
    blob = BlobAnalyzer()
    results = blob.analyze(image, roi=(x,y,w,h))
"""
...
class BlobDetector:
    ...
    def _compute_features(
        ...
        if len(contour) >= 5:
            try:
                (_, 2), eigenvalues = cv2.fitEllipse(contour)
...
class GrayMatcher:
    ...
    def __init__(
        ...
        Args:
            ...
            angle_range:   旋转搜索范围
            angle_step:    旋转搜索步长
            match_method:  匹配方法，'tm_ccoeff_normed'（推荐）
        """
    ...
    def _match_at_angle(
        ...
        # 归一化到 [0, 100]
        result = np.clip(result * 100, 0, 100)
...
def match_gray(
    ...
    **kwargs,
    ...
    return matcher.match(image, num_matches=kwargs.get('num_matches', 1))
